"""
    Example of sqlite session.
    Visit http://flask.pocoo.org/snippets/86/
    Implementation:
        session -> a database named sid(sessionId), where there is a table called 'session'
        Each user should have his/her own session db file.
        And every user should have unique sid.
"""
...
class SqliteSession(MutableMapping, SessionMixin):
    ...
    def __init__(self, directory, sid, *args, **kwargs):
        ...
        self.sid = sid
        self.modified = False   # if db is modified after loading
        self.conn = None

        # initiate if not exist -> create database and session table
